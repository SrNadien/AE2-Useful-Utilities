 ...
 */
public record ProviderKey(Identifier dimension, BlockPos pos, int side) {

    public static final int NO_SIDE = -1;

    public static final StreamCodec<FriendlyByteBuf, ProviderKey> STREAM_CODEC = StreamCodec.of(
            (buf, key) -> {
                buf.writeIdentifier(key.dimension());
                buf.writeBlockPos(key.pos());
                buf.writeByte(key.side());
            },
            buf -> new ProviderKey(buf.readIdentifier(), buf.readBlockPos(), buf.readByte()));

    public static ProviderKey of(Identifier dimension, BlockPos pos, @Nullable Direction side) {
